class ActionCheckSCtable_drawer: ActionInteractBase
{
	ref Timer		m_CheckSCtable_drawerTimer;

	void ActionCheckSCtable_drawer()
	{
	//	m_MessageSuccess = "";
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		SCtable_drawer m_CheckRandomLoot = SCtable_drawer.Cast( targetObject );

		if ( m_CheckRandomLoot && m_CheckRandomLoot.CanLootSCtable_drawer() )
		{
			return true;
		}
		return false;
	...
	override void OnStartServer( ActionData action_data )
	{
		...
		SCtable_drawer GarbageTimerCast = SCtable_drawer.Cast( targetObject );
		GarbageTimerCast.Garbage_BinTimerStart()
	 	...
	 	switch (Math.RandomInt(0, 5)) 
        {
			...
			switch (Math.RandomInt(0, 3))
				{
    				case 0:
						ItemBase Garbage3 = ItemBase.Cast( GetGame().CreateObject("ChernarusMap", pos) );
			...
			case 3:
				...
				{
    				case 0:
						ItemBase Garbage9 = ItemBase.Cast( GetGame().CreateObject("Marmalade", pos) );
						SetItemQuantity9(Garbage9);
            		break; 
					
           			case 1:
						ItemBase Garbage10 = ItemBase.Cast( GetGame().CreateObject("MoneyRuble5", pos) );
			...
			case 4:
				switch (Math.RandomInt(0, 7))
				{
            		...
            		break; 
           			...
           			case 1:
						ItemBase Garbage14 = ItemBase.Cast( GetGame().CreateObject("Nail", pos) );
						SetItemQuantity14(Garbage14);
            		break;
					
					case 2:
						ItemBase Garbage16 = ItemBase.Cast( GetGame().CreateObject("CombinationLock4", pos) );
            		break;
