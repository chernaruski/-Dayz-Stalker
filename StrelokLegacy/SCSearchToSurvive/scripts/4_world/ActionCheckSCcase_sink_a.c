class ActionCheckSCcase_sink_a: ActionInteractBase
{
	ref Timer		m_CheckSCcase_sink_aTimer;

	void ActionCheckSCcase_sink_a()
	{
	//	m_MessageSuccess = "";
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		SCcase_sink_a m_CheckRandomLoot = SCcase_sink_a.Cast( targetObject );

		if ( m_CheckRandomLoot && m_CheckRandomLoot.CanLootSCcase_sink_a() )
		{
			return true;
		}
		return false;
	...
	override void OnStartServer( ActionData action_data )
	{
		...
		SCcase_sink_a Garbage_BinTimerCast = SCcase_sink_a.Cast( targetObject );
	 	...
	 	switch (Math.RandomInt(0, 5)) 
        {
            case 0:
				switch (Math.RandomInt(0, 2))
				{
            		...
            		break; 
           			...
           			case 1:
						ItemBase Garbage2 = ItemBase.Cast( GetGame().CreateObject("KitchenKnife", pos) );
			...
			case 3:
				...
				{
    				case 0:
						ItemBase Garbage9 = ItemBase.Cast( GetGame().CreateObject("Pot", pos) );
						SetItemQuantity9(Garbage9);
            		break; 
					
           			case 1:
						ItemBase Garbage10 = ItemBase.Cast( GetGame().CreateObject("DuctTape", pos) );
			...
			case 4:
				switch (Math.RandomInt(0, 7))
				{
					...
					case 2:
						ItemBase Garbage16 = ItemBase.Cast( GetGame().CreateObject("SmallGasCanister", pos) );
            		break;
